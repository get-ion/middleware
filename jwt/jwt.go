...
// ion provides some basic middleware, most for your learning courve.
// You can use any net/http compatible middleware with ion.ToHandler wrapper.
//
...
// (because it used "context" to define the user but we don't need that so a simple ion.ToHandler wouldn't work as expected.)
// jwt_test.go also didn't created by me:
// 28 Jul 2016
// @heralight heralight add jwt unit test.
//
// So if this doesn't works for you just try other net/http compatible middleware and bind it via `ion.ToHandler(myHandlerWithNext)`,
...
func (m *Middleware) CheckJWT(ctx context.Context) error {
	if !m.Config.EnableAuthOnOptions {
		if ctx.Method() == ion.MethodOptions {
			return nil
		}
	}
