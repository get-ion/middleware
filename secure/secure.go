// This has been cloned to work with ion,
...
// Serve implements the context.HandlerFunc for integration with ion.
...
func (s *Secure) Process(ctx context.Context) error {
	...
	if s.opt.SSLRedirect && !isSSL && !s.opt.IsDevelopment {
		...
		status := ion.StatusMovedPermanently
		if s.opt.SSLTemporaryRedirect {
			status = ion.StatusTemporaryRedirect
		}
